...
### MuJoCo Simulation (full stack)

```bash
# Default: MuJoCo viewer + RViz
ros2 launch so_arm101_control mujoco.launch.py

# Without RViz
ros2 launch so_arm101_control mujoco.launch.py rviz:=false

# Headless (no viewer window)
ros2 launch so_arm101_control mujoco.launch.py headless:=true rviz:=false
```

Launches MuJoCo for physics visualization and camera rendering, ros2_control (mock hardware), MoveIt, control GUI, and pose publishers. The MuJoCo viewer shows the robot + lego bricks.

```bash
# Randomize lego positions (visible in camera)
ros2 run so_arm101_control randomize_legos

# Reset to defaults
ros2 run so_arm101_control randomize_legos --reset
```

### Gazebo Simulation (Ignition + full stack) [Legacy]
...
Spawns the robot in Ignition Gazebo with physics, loads ros2_control controllers, and launches the full stack (MoveIt + GUI + RViz). Requires `ros-humble-ros-gz-*` and `ros-humble-gz-ros2-control` packages.
